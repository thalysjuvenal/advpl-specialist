...
### Added / Adicionado
- New knowledge base `skills/smartview-development/` documenting **Smart View** custom business objects (Integrated Provider) — a gap in previous versions, where Smart View appeared only in `advpr-test-automation` (test automation, not construction): the minimum contract (`@totvsFrameworkTReportsIntegratedProvider`, `new`/`getSchema`/`getData`), environment prerequisites, object identity (source name = id used by `callTReports` and as the `.trp` prefix), schema patterns (types, dates via `stringToTimeStamp`, calculated and discriminator columns, page size), parameter patterns (`addParameter`, `setPergunte`, advanced search via `genericLookupService`, combos requiring a custom options endpoint with an explicit allow-list), `getData` patterns (BeginSQL for static queries vs `FWExecStatement` with binds for runtime-built `WHERE`, why `%xfilial%` must not be used, multi-branch, currency, volume), report migration (TReport/FWMSPrinter/TMSPrinter/FWMSExcel de/para, what has no conversion path, the two silent bugs — unpadded "to" ranges and dates published as text), the `.trp` publication cycle and troubleshooting. New command `/advpl-specialist:smartview` generates or migrates Smart View business objects (`--mode generate|migrate`, `--type data-grid|reports|pivot-table`). Contributed by Henrique Patriota ([@suportem3](https://github.com/suportem3), M3 Case) via [PR #16](https://github.com/thalysjuvenal/advpl-specialist/pull/16).
- Nova base de conhecimento `skills/smartview-development/` documentando objetos de negocio **Smart View** customizados (Integrated Provider) — lacuna das versoes anteriores, em que Smart View aparecia apenas na `advpr-test-automation` (automacao de teste, nao construcao): contrato minimo (`@totvsFrameworkTReportsIntegratedProvider`, `new`/`getSchema`/`getData`), pre-requisitos de ambiente, identidade do objeto (nome do fonte = id usado no `callTReports` e como prefixo do `.trp`), padroes de schema (tipos, datas via `stringToTimeStamp`, colunas calculadas e discriminadora, tamanho de pagina), padroes de parametros (`addParameter`, `setPergunte`, busca avancada pelo `genericLookupService`, combos que exigem endpoint de opcoes proprio com lista explicita), padroes de `getData` (BeginSQL para consulta estatica x `FWExecStatement` com bind para `WHERE` montado em tempo de execucao, por que nao usar `%xfilial%`, multi-filial, moeda, volume), migracao de relatorios (de/para TReport/FWMSPrinter/TMSPrinter/FWMSExcel, o que nao tem caminho de conversao, os dois bugs silenciosos — faixas "ate" sem padding e datas publicadas como texto), ciclo de publicacao do `.trp` e troubleshooting. Novo comando `/advpl-specialist:smartview` gera ou migra objetos de negocio Smart View (`--mode generate|migrate`, `--type data-grid|reports|pivot-table`). Contribuicao de Henrique Patriota ([@suportem3](https://github.com/suportem3), M3 Case) via [PR #16](https://github.com/thalysjuvenal/advpl-specialist/pull/16).
...
### Added / Adicionado
- New `skills/advpl-code-generation/patterns-fwmsprinter.md` — complete reference for generating coordinate-based PDF reports with `FWMSPrinter`. Covers constructor (12-parameter signature with `IMP_PDF`, `cPathPDF`, `SetResolution`), setup methods (`SetLandscape`, `SetPaperSize`, `SetMargin`), drawing primitives (`SayAlign` with alignment constants `PAD_LEFT/RIGHT/CENTER/JUSTIFY`, `Line`, `FillRect` via `TBrush`, `SayBitmap`), supporting classes (`TFont` with negative-height convention, `TBrush` with mandatory `End()` call), layout strategies (cumulative column array, page-break guard, two-row record pattern), A4-landscape pixel width reference table, a complete working example (BeginSQL/EndSQL query, `Begin Sequence`/`Recover` error handling, `FWGetArea`/`FWRestArea`, `Protheus.doc` headers on all functions, `fFillRect` helper encapsulating `TBrush`), 5 common patterns (colored header, zebra striping, blank fields, totals row, two-level headers), and an error reference table with confirmed runtime error messages and fixes. All signatures verified against Protheus build 7.00.240223P. Contributed by Henrique Patriota ([@suportem3](https://github.com/suportem3), M3 Case) via [PR #12](https://github.com/thalysjuvenal/advpl-specialist/pull/12).
- Novo `skills/advpl-code-generation/patterns-fwmsprinter.md` — referência completa para gerar relatórios PDF por coordenadas com `FWMSPrinter`. Cobre constructor (assinatura de 12 parâmetros com `IMP_PDF`, `cPathPDF`, `SetResolution`), métodos de setup (`SetLandscape`, `SetPaperSize`, `SetMargin`), primitivas de desenho (`SayAlign` com constantes `PAD_LEFT/RIGHT/CENTER/JUSTIFY`, `Line`, `FillRect` via `TBrush`, `SayBitmap`), classes de apoio (`TFont` com convenção de altura negativa, `TBrush` com chamada obrigatória de `End()`), estratégias de layout (array de colunas cumulativo, guarda de quebra de página, layout de duas linhas por registro), tabela de referência de larguras em pixels para A4 paisagem, exemplo completo funcional (query com `BeginSQL`/`EndSQL`, tratamento de erros com `Begin Sequence`/`Recover`, `FWGetArea`/`FWRestArea`, cabeçalhos `Protheus.doc` em todas as funções, helper `fFillRect` encapsulando `TBrush`), 5 padrões comuns (cabeçalho colorido, zebra striping, campos em branco, linha de totais, cabeçalhos em dois níveis) e tabela de referência de erros com mensagens de runtime confirmadas e correções. Todas as assinaturas verificadas contra o build Protheus 7.00.240223P. Contribuição de Henrique Patriota ([@suportem3](https://github.com/suportem3), M3 Case) via [PR #12](https://github.com/thalysjuvenal/advpl-specialist/pull/12).
